...
import ca.bc.gov.mal.pit.common.rest.resource.transformers.JsonTransformer;
...
public abstract class BaseRestServiceClient {
	...
	private static final int DefaultMaxInMemorySize = 36 * 1024 * 1024;
	...
	protected void init(ExchangeFilterFunction exchangeFilter, int maxInMemorySize) {
		...
		this.restDAOFactory = initRestDAOFactory();
		this.transformer = initTransformer();
	}
	
	protected RestDAOFactory initRestDAOFactory() {
		RestDAOFactory result;
		
		result = new RestDAOFactory(getClientVersion());
	...
	public void setTopLevelRestURL(String topLevelRestURL) {
		...
		if(topLevelRestURL!=null&&!topLevelRestURL.endsWith("/")) {
	...
	public String getSwaggerString() throws RestClientServiceException {
		...
		try {
			...
			Response<String> restResponse = dao.Process(
					BaseResourceTypes.SELF, getTransformer(), new BaseResource() {
	...
	public HealthCheckResponseRsrc getHealthCheck(String callstack) throws RestClientServiceException {
		...
		try {
			...
			Response<HealthCheckResponseRsrc> response = dao.Process(
					BaseResourceTypes.HEALTH_CHECK_RESPONSE, getTransformer(), new BaseResource() {
